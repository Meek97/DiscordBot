...
module.exports = {
	...
	async execute(interaction) {
		...
		if (results == null) {
			...
			const numResults = results.args.length;
			if (numResults == 1) {
				row.components[0].setDisabled(true);
				row.components[1].setDisabled(true);
			}
			const messageHeader = `\`${temp}\`|1|${numResults}\n`;
			await interaction.reply({ content: messageHeader + results.args[0], components: [row], ephemeral: true });
		}
	},
	async next(interaction, _key, index, modifier) {
		...
		if (results == null) {
			...
			await interaction.update({ content: messageHeader + results.args[index - 1] });
		}
	},
	async delete(interaction, _key, index) {
		...
		if (results == null) {
			...
			results.args.splice((index - 1), 1);
			const edit_results = await mongoDriver.UpdateOneDocument(
				{ key:_key },
				{ $set: { args : results.args } },
				SUBMISSIONS_DB);
